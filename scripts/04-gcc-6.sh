...
DATA
sudo sed -i 's/GCC_VERSION/'"$GCC_VERSION"'/g' /etc/portage/package.unmask/gcc
sudo cat /etc/portage/package.unmask/gcc

# DEBUG: check if version is correctly set
sudo cat /etc/portage/package.unmask/gcc

sudo emerge --oneshot sys-devel/gcc:${GCC_VERSION}

# DEBUG:
sudo gcc-config --list-profiles

sudo gcc-config "x86_64-pc-linux-gnu-${GCC_VERSION}"
